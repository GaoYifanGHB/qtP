...
string saveSpot::readSpotPoint(int &readNum,int *num,float readP[300][2],int mark){
    ...
    *num=stringToNum<int>(numS);
    ...
    for(int i=1;i<*num+1;i++){
        ...
        //乘以4,画的时候缩小了4倍  -------------------测试  xiaotian 画的时候压缩了4
        if(digitalFlag == 1){
            tempSX*=4;
            tempSY*=4;
        }
